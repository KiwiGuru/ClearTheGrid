...
namespace StarterKit
{
    ...
    internal class Program
    {
            ...
            for (int level = 1; level < 100; level++)
            {
                ...
                while (!isDone)
                {
                    ...
                    else
                    {
                        ...
                        //TODO check with actualmovecount
                        //moves[rnd.Next(moves.Count)] = new Move(rnd.Next(currentMap.w), rnd.Next(currentMap.h), (Direction)rnd.Next(4),rnd.Next(2)==1);
                    }
                }
            }
        }
        ...
        static List<Move> GetValidMoves(Map map)
        {
            ...
            //TODO See if this can be done only cycling coordinates which have numbers (Hashmap of known numbers and coordinates?)
            for (int x = 0; x < map.w; x++)
            {
                for (int y = 0; y < map.h; y++)
                {
